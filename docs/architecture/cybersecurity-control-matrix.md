...
| G-7 | **Web application firewall rule baseline and tuning** — which managed rule sets are enabled and how false positives are handled | TASK-021 | CTL-04 stands up the WAF; no rule baseline is stated. | Engineering value, not an AHDA value; TASK-021 records the baseline — **recorded**: eight OWASP CRS 3.3 rule sets at sensitivity 1, enforcing in all four environments, and the false-positive procedure (`network-security.md` §3.4) |
