...
| Date | Change | By |
| --- | --- | --- |
| 2026-09-25 | `backend` gains a PostgreSQL 17 service container and a step that runs the migration rollback tests (`PMPlatform.Tests.Integration.Persistence` only). This supersedes the §2 "Integration tests" bullet for those tests: they need a database server, not the containerised stack, and they run in about a second. `repo-checks` checks out full history (`fetch-depth: 0`) and, on pull requests, runs `migration-forward-only.sh`. No required check was renamed or added. Record: `database-migrations.md` §3.5. | Database (TASK-024) |
