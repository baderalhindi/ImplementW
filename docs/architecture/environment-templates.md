...
| **R-4** The frontend template holds Public rows only, as `VITE_<NAME>` | Five variables: `VITE_APP_BASE_URL`, `VITE_SSO_OIDC_AUTHORITY`, `VITE_SSO_OIDC_CLIENT_ID`, `VITE_SSO_OIDC_CALLBACK_URL`, `VITE_APM_ENVIRONMENT_TAG`. The prefix marks the same sheet row; the check strips it. | Everything in a Vite bundle is public once built (ADR-003 T-1); `vite.config.ts` already exposes only `VITE_`. A Secret-classified row in `.env.example` would be a design error, and the check treats it as one. The SSO three are conditional on TASK-028 (§6 F-3). |
